import { buildReferenceIndex, type ReferenceIndex } from "./referenceIndex.ts";
import { selectEvidence, type WorkPermitRuntime } from "./evidence.ts";
import {
  getLastMessage,
  getPreviousUserMessage,
  type ChatMsg,
} from "./text.ts";
import { APP_SKILL_PROMPT, BASE_POLICY_PROMPT } from "./workPermitConfig.ts";
import {
  classifyIntent,
  detectPendingSlot,
  inferAreaFromText,
...
export interface ChatPlanDirect {
  directResponse: string;
}

export interface ChatPlanMessages {
  messages: ChatMsg[];
}

export type ChatPlan = ChatPlanDirect | ChatPlanMessages;

export function buildWorkPermitChatPlan(messages: ChatMsg[], referenceText: string): ChatPlan {
  ...
  const runtime: WorkPermitRuntime = {
    intent,
    slots,
    pendingSlot,
    fallbackMode,
  ...
  const mandatoryFollowUp =
    fallbackMode || wantsGenericExplanation(latestUserMessage) || hasFaqMatch
      ? undefined
