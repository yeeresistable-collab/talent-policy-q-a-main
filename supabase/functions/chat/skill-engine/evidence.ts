import type { ReferenceChunk, ReferenceIndex } from "./referenceIndex.ts";
...
export interface WorkPermitRuntime {
  intent: string;
  slots: RuntimeSlots;
  pendingSlot?: string;
  fallbackMode: boolean;
  semanticQuestion: string;
  isAClassDefinition: boolean;
}

function hasAnyTag(chunk: ReferenceChunk, tags: string[]) {
  return tags.some((tag) => chunk.tags.includes(tag));
}

function normalizeForMatch(text: unknown) {
...
function hasFaqLikeQuestion(text: string) {
  return /[？?]|是否|需不需要|需要.*吗|要不要|能不能|可不可以/.test(text);
}

function extractQueryTokens(text: unknown) {
  return Array.from(
    new Set(
      String(text ?? "")
        .replace(/[，。！？、；：“”"'（）()\[\]{}<>《》]/g, " ")
...
function isHighConfidenceFaqMatch(chunk: ReferenceChunk, question: string) {
...
function businessTags(slots: RuntimeSlots) {
  const tags: string[] = [];
...
function intentTags(intent: string) {
...
function scoreChunk(chunk: ReferenceChunk, runtime: WorkPermitRuntime) {
  let score = 0;
  ...
  if (
    runtime.fallbackMode &&
    hasAnyTag(chunk, ["employer", "longTerm", "shortTerm", "extension", "change", "cancellation", "windows"])
  ) {
    score += 35;
  }
...
export function selectEvidence(referenceIndex: ReferenceIndex, runtime: WorkPermitRuntime) {
