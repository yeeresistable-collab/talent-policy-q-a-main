import { areaMatches, formatWindowRows } from "./referenceIndex.js";
import { FINAL_NOTICE } from "./workPermitConfig.js";
import { isUnclearReply, wantsAllWindows } from "./workPermitNlu.js";

export function buildMandatoryFollowUp(intent, slots, userText) {
  if (intent === "windows" && !slots.current_area && !isUnclearReply(userText) && !wantsAllWindows(userText)) {
    return {
      missingSlot: "current_area",
      question: "为了按就近原则推荐办理窗口，请先告诉我您目前在哪个区、哪条路、哪个地铁站、哪个商圈，或公司大致地址。",
    };
  }
  ...
  if (intent === "materials") {
    if (!slots.business_type) {
      return {
        missingSlot: "business_type",
        question: "材料清单需要先对应具体业务类型。请先确认您要办理的是新办、延期、变更、注销，还是用人单位注册？",
      };
    }
...
export function buildWindowDirectResponse(referenceIndex, runtime) {
...
export function buildShortTermDirectResponse(referenceIndex, runtime) {
