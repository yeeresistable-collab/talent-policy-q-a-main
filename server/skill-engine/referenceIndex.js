...
export function buildReferenceIndex(referenceText) {
...
export function splitReferenceIntoChunks(referenceText) {
...
export function getChunkTags(title, content) {
  const text = `${title}\n${content}`;
  const tags = new Set();
...
export function parseWindowRows(referenceText) {
  const rows = [];
  const lines = referenceText.split(/\r?\n/);
...
export function areaMatches(rowArea, currentArea) {
...
export function formatWindowRows(rows) {
