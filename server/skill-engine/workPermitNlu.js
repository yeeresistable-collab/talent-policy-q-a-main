import { WORK_PERMIT_SKILL_CONFIG } from "./workPermitConfig.js";
import { applyAliases, getUserMessages, normalizeText } from "./text.js";

export function normalizeSkillText(text) {
  return applyAliases(text, WORK_PERMIT_SKILL_CONFIG.aliases);
}

export function isWorkPermitQuestion(text) {
...
export function classifyIntent(text) {
  const compact = normalizeSkillText(text);
  if (/窗口|站点|哪里办|哪里办理|哪里可以办理|在哪办|在哪里办|去哪办|去哪里办|地址|地点|位置|电话|最近|服务中心|受理机构/.test(compact)) {
    return "windows";
  }
...
export function inferSlots(messages) {
  ...
  const areaMatch = /(东城|西城|朝阳|海淀|丰台|石景山|房山|通州|顺义|昌平|大兴|怀柔|延庆|经开|亦庄|[^，。！？\s]{2,12}(?:路|街|地铁站|商圈))/.exec(text);
...
export function inferAreaFromText(text) {
  ...
  const areaMatch = /(东城|西城|朝阳|海淀|丰台|石景山|房山|通州|顺义|昌平|大兴|怀柔|延庆|经开|亦庄|[^，。！？\s]{2,12}(?:路|街|地铁站|商圈))/.exec(normalized);
  return areaMatch ? areaMatch[1] : undefined;
}

export function normalizeSlotsForIntent(slots, intent) {
...
export function isAClassDefinition(text) {
  const compact = normalizeSkillText(text);
  return /A\s*类|A类|外国高端人才/.test(compact) && /什么|定义|认定|条件|判断|标准|属于/.test(compact);
}

export function isUnclearReply(text) {
  return /不清楚|不知道|不确定|不明白|不太清楚|不懂|没法判断|无法判断/.test(normalizeSkillText(text));
}

export function wantsGenericExplanation(text) {
  return /通用|先讲|先说|大概|概览|原则|不管.*情况|都列|全部/.test(normalizeSkillText(text));
}

export function wantsAllWindows(text) {
  return /各区|不同区域|所有|全部|全量|汇总|列表|一览|有哪些|都有哪些|任何一个区|任意区|任一区|哪个区都行|都可以|随便哪个区/.test(normalizeSkillText(text));
}

export function detectPendingSlot(assistantText) {
